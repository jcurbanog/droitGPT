...
class VectorDatabase:
    ...
    def create_or_load(self):
        if (
            os.path.exists(self.vector_db_path) and os.path.isdir(self.vector_db_path)
...
class droitGPT:
    def __init__(self, llm_pipe: HuggingFacePipeline, vector_database: VectorDatabase):
        ...
        Voici la conversation entre vous et l'utilisateur:
        {conversation}
        {context}
        user: {input}
        assistant:
    ...
    def create(self):
        ...
        retriever_chain = create_history_aware_retriever(
            self.llm_pipe, self.retriever, prompt_context
        )
    ...
    def build_chat_history(conversation: List[Tuple[str, str]]) -> List[HumanMessage | AIMessage]:
        return [
            (
                HumanMessage(content=speaker_text["text"])
                if speaker_text["speaker"] == "user"
                else AIMessage(content=speaker_text["text"])
            )
            for speaker_text in conversation
        ]
    ...
    def answer(
        ...
        outputs = [
            self.retrieval_chain.invoke({"chat_history": chat_history, "input": input})
            for _ in range(n)
        ]
...
def droitGPT_init() -> droitGPT:
    llm_pipe_factory = LLMPipelineFactory(
        model_id=Config.LLM_MODEL_ID, max_new_tokens=Config.LLM_MAX_NEW_TOKENS
    )
